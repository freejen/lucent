...
class BottleneckMLP(nn.Module):
    def __init__(
        self, dim_in, dim_out, block_dims, norm="layer", checkpoint=None, name=None
    ):
    ...
    def load(self, name, checkpoint_path="./checkpoints/"):
        ...
        params = {k: v for k, v in torch.load(weight_path).items()}
...
def get_model(architecture, checkpoint, resolution, num_classes):
    return model_list[architecture](
        dim_in=resolution**2 * 3, dim_out=num_classes, checkpoint=checkpoint
    )
